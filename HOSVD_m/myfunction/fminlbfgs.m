function [x,fval_total_obj,exitflag,output,gNorm]=fminlbfgs(funfcn,x_init,optim)
...
%	- Quasi Newton Broyden朏letcher朑oldfarb朣hanno (BFGS)  
...
%	    OPTIONS.HessUpdate : If set to 'bfgs', Broyden朏letcher朑oldfarb朣hanno 
...
defaultopt = struct('Display','final','HessUpdate','bfgs','GoalsExactAchieve',1,'GradConstr',true,  ...
			'TolX',1e-8,'TolFun',1e-6,'GradObj','off','MaxIter',6,'MaxFunEvals',100*numel(x_init)-1,  ...
...
while(true)
    ...
    if ismember(data.iteration,[100 120 140 160 180 200 220 240 260 280 300 350 400 450 500])
       x = data.xInitial;
    ...
    if(strcmp(optim.Display(1),'i')||strcmp(optim.Display(1),'p'))
        s=sprintf('     %5.0f                  %5.0f                      %5.0f                  %13.6g                        %13.6g                       %13.6g',data.iteration,data.funcCount,data.gradCount,data.fInitial,gNorm(iii),data.alpha); 
        disp(s);
    end
...
if(optim.HessUpdate(1)=='b'), output.algorithm='Broyden朏letcher朑oldfarb朣hanno (BFGS)';
